...
import { SendOutlined, RobotOutlined, LoadingOutlined, CloseOutlined, UserOutlined, MessageOutlined } from '@ant-design/icons';
    ...
    return (
        <AnimatePresence>
            {isVisible && (
                ...
                >
                    <div className="flex flex-col h-full relative bg-white sm:rounded-2xl shadow-2xl overflow-hidden backdrop-blur-sm bg-opacity-95" ref={chatbotRef}>
                        {/* Header */}
                        <motion.div 
                            initial={{ y: -20 }}
                            animate={{ y: 0 }}
                        ...
                        {/* Chat Messages */}
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-gray-50 to-white" style={{ height: "calc(100% - 120px)" }}>
                            {messages.map((chat, index) => (
                                chat.message.map((msg, msgIndex) => (
                                    ...
                                    >
                                        ...
                                        >
                                            {msg.role === 'assistant' && <RobotOutlined className="mt-1 text-[#F17905]" />}
                                            <div className="flex-1 text-sm sm:text-base">
                                                <FormattedMarkdown>{msg.content}</FormattedMarkdown>
                                            </div>
                                            {msg.role === 'user' && <UserOutlined className="mt-1 text-white" />}
                                        </div>
                                    </motion.div>
                                ))
                            ))}
