  ...
  if (!started) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[300px] sm:h-[400px] bg-gradient-to-br from-orange-50 to-white rounded-xl shadow p-4 sm:p-6">
        ...
        >
          <h1 className="text-xl sm:text-2xl font-bold text-[#E06901] mb-2">
      ...
      <div className="bg-white rounded-xl shadow p-4 sm:p-6">
        ...
        <div className="space-y-2 sm:space-y-3">
          {questions[currentQuestion].options.map((option, index) => {
            ...
            const reveal = showExplanation; // when true, reveal correct option

            const baseClasses =
              "w-full p-3 sm:p-4 rounded-lg text-left text-sm sm:text-base transition-all duration-200";
            ...
            if (reveal) {
              if (isCorrectOption) {
                optionClasses =
                  "bg-green-50 border border-green-400 text-gray-800";
              } else if (isSelected && !isCorrectOption) {
                optionClasses =
                  "bg-red-50 border border-red-400 text-gray-800 line-through/0";
              } else {
                optionClasses = "bg-gray-50 text-gray-500";
              }
            } else if (isSelected) {
              optionClasses = isCorrectOption
                ? "bg-green-100 border border-green-500 text-gray-800"
                : "bg-red-100 border border-red-500 text-gray-800";
            }
            ...
            return (
              <motion.button
                ...
                className={`${baseClasses} ${optionClasses} flex items-center justify-between`}
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <span
                    className={`${
                      reveal && !isCorrectOption ? "opacity-80" : ""
                    }`}
                  >
                    {option}
                  </span>
                ...
                <div className="flex items-center gap-2">
                  {/* show icon when revealed or when selected */}
        ...
        {showHint && !showExplanation && (
          <div className="mt-4 p-3 sm:p-4 bg-orange-50 rounded-lg border border-orange-200">
            <p className="text-[#E06901] text-xs sm:text-sm">
